import sys
sys.path.append("/opt/data/private/adder-transformer")
from operation.adder import OperateBase, ConvBase
from model.resnet20 import get_model
...
if __name__ == "__main__":
    ...
    net = get_model(operate_base = None,
                    conv_base = None
                    ).to(device)
    ...
    # use Adam optimizer
    optimizer = optim.SGD([{'params': net.parameters(), 'initial_lr':learning_rate}], 
    ...
    # resume
    if resume:
        checkpoint = torch.load("exp_96.state.pt")
        net.load_state_dict(checkpoint["model"])
        optimizer.load_state_dict(checkpoint["optimizer"])
        scaler.load_state_dict(checkpoint["scaler"])
        scheduler = torch.optim.lr_scheduler.LambdaLR(optimizer, lambda x: 0.1-0.001*x )
