...
def conv_1x1_bn(inp, oup):
    return nn.Sequential(
        ConvBase(inp, oup, 1, 1, 0, bias=False),
        nn.BatchNorm2d(oup),
        nn.SiLU(), )
def conv_nxn_bn(inp, oup, kernel_size=3, stride=1):
    return nn.Sequential(
        ConvBase(inp, oup, kernel_size, stride, 1, bias=False),
        nn.BatchNorm2d(oup),
        nn.SiLU(), )
...
class FeedForward(nn.Module):
    def __init__(self, dim, hidden_dim, dropout=0.):
        ...
        self.net = nn.Sequential(
            nn.LayerNorm(dim),
            OperateBase(dim, hidden_dim),
            nn.SiLU(),
            nn.Dropout(dropout),
            OperateBase(hidden_dim, dim),
            nn.Dropout(dropout), )
    def forward(self, x):
        return self.net(x)
...
class Attention(nn.Module):
    def __init__(self, dim, heads=8, dim_head=64, dropout=0.):
        ...
        self.to_qkv = OperateBase(dim, inner_dim * 3, bias=False)
    ...
    def __init__(self, inp, oup, stride=1, expansion=4):
        ...
        if expansion == 1:
            self.conv = nn.Sequential(
                # dw
                ConvBase(hidden_dim, hidden_dim, 3, stride, 1, groups=hidden_dim, bias=False),
                nn.BatchNorm2d(hidden_dim),
                nn.SiLU(),
                # pw-linear
                ConvBase(hidden_dim, oup, 1, 1, 0, bias=False),
                nn.BatchNorm2d(oup),
            )
        else:
            self.conv = nn.Sequential(
                # pw
                ConvBase(inp, hidden_dim, 1, 1, 0, bias=False),
                nn.BatchNorm2d(hidden_dim),
                nn.SiLU(),
                # dw
                ConvBase(hidden_dim, hidden_dim, 3, stride, 1, groups=hidden_dim, bias=False),
...
class MobileViT(nn.Module):
    ...
    def __init__(
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(3, init_dim, 3, 2, 1, bias=False),
        ...
        self.to_logits = nn.Sequential(
            ...
            OperateBase(channels[-1], num_classes, bias=False)
        )
