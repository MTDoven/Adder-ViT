...
Codes: https://github.com/facebookresearch/LeViT
"""
...
class FeedForward(nn.Module):
    def __init__(self, dim, mult, dropout = 0.):
        ...
        self.net = nn.Sequential(
            ConvBase(dim, dim * mult, 1),
            nn.Hardswish(),
            nn.Dropout(dropout),
            ConvBase(dim * mult, dim, 1),
            nn.Dropout(dropout), )
    def forward(self, x):
        return self.net(x)
...
class Attention(nn.Module):
    def __init__(self, dim, fmap_size, heads = 8, dim_key = 32, dim_value = 64, dropout = 0., dim_out = None, downsample = False):
        ...
        self.to_q = nn.Sequential(ConvBase(dim, inner_dim_key, 1, stride = (2 if downsample else 1), bias = False), nn.BatchNorm2d(inner_dim_key))
        self.to_k = nn.Sequential(ConvBase(dim, inner_dim_key, 1, bias = False), nn.BatchNorm2d(inner_dim_key))
        self.to_v = nn.Sequential(ConvBase(dim, inner_dim_value, 1, bias = False), nn.BatchNorm2d(inner_dim_value))
        ...
        self.to_out = nn.Sequential(
            nn.GELU(),
            ConvBase(inner_dim_value, dim_out, 1),
            out_batch_norm,
            nn.Dropout(dropout), )
    ...
    def __init__(self, dim, fmap_size, depth, heads, dim_key, dim_value, mlp_mult = 2, dropout = 0., dim_out = None, downsample = False):
        ...
        for _ in range(depth):
            self.layers.append(nn.ModuleList([
                Attention(dim, fmap_size = fmap_size, heads = heads, dim_key = dim_key, 
                          dim_value = dim_value, dropout = dropout, downsample = downsample, dim_out = dim_out),
    ...
    def __init__(
        ...
        *,
        ...
        self.conv_embedding = nn.Sequential(
            ConvBase(3, 32, 3, stride = 2, padding = 1),
            ConvBase(32, 64, 3, stride = 2, padding = 1),
            ConvBase(64, 128, 3, stride = 2, padding = 1),
            ConvBase(128, dims[0], 3, stride = 2, padding = 1), )
...
OperateBase = None
ConvBase = None
def get_model(operate_base, conv_base, **kwargs):
    global OperateBase
    global ConvBase
